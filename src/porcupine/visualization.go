      ...
      function render(data) {
        ...
        data.forEach(partition => {
          partition['History'].forEach(el => {
            maxClient = Math.max(maxClient, el['ClerkID'])
          })
        })
        ...
        data.forEach((partition, partitionIndex) => {
          ...
          partition['History'].forEach((el, elIndex) => {
            ...
            const y = PADDING + el['ClerkID'] * (BOX_HEIGHT + BOX_SPACE)
        ...
        data.forEach((partition, partitionIndex) => {
          ...
          partition['PartialLinearizations'].forEach((lin, linIndex) => {
            ...
            lin.forEach(id => {
              ...
              const y = PADDING + el['ClerkID'] * (BOX_HEIGHT + BOX_SPACE) - LINE_BLEED
              // line from previous
              if (prevEl !== null) {
                svgadd(g, 'line', {
                  'x1': prevX,
                  'x2': x,
                  'y1': prevEl['ClerkID'] >= el['ClerkID'] ? prevY : prevY + BOX_HEIGHT + 2*LINE_BLEED,
                  'y2': prevEl['ClerkID'] <= el['ClerkID'] ? y : y + BOX_HEIGHT + 2*LINE_BLEED,
